...
/**
 * Statistics for patch script sizes per time interval.
...
public class PatchScriptSizePerInterval extends IntervalMap<Long> {
    ...
    public void receive(Commit rc, LocalDate truncStart) {
        Long previous = getMap().getOrDefault(truncStart, 0L);
        getMap().put(truncStart, previous + rc.getPatchScriptSize().get());
    }
